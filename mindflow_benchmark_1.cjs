...
const EVAL_SYSTEM = `Tu es un evaluateur expert en methode Pastek. Ta reponse doit etre UNIQUEMENT le JSON ci-dessous, sans aucun texte avant ou apres, sans backticks, sans markdown.

REPONDRE EXACTEMENT AVEC CE FORMAT :
{"engagement":7,"progression":6,"methode_globale":7,"attention":8,"tri_selectif":6,"effet_miroir":7,"canal_secondaire":5,"coherence_passion":7,"violations":["exemple violation"],"obs":"observation courte"}

Criteres de notation (0-10) :
engagement : l'eleve s'engage-t-il vraiment ? (posture, qualite reponses, initiative)
progression : comprehension croissante visible ?
methode_globale : respect global Pastek (tri selectif, une question, effet miroir)
attention : reste dans le sujet ?
tri_selectif : Dr Mind evalue ce que l'eleve sait AVANT d'avancer ?
effet_miroir : Dr Mind reformule avec mots de l'eleve enrichis AVANT de relancer ?
canal_secondaire : Dr Mind active un second canal sensoriel (pas juste le dominant) ?
coherence_passion : les analogies correspondent vraiment a la passion de l'eleve ?
violations : liste des violations Pastek (double question, reponse directe, validation je sais rien, derive counseling) - tableau vide si aucune
obs : observation courte ou null

IMPORTANT : commence directement par { et termine par }. Aucun texte avant ou apres.`;
  ...
  for (let i = 0; i < nbTours; i++) {
    ...
    try {
      ...
      Object.keys(scores).forEach(k => {
        const val = ev[k];
        if (typeof val === 'number' && val >= 0 && val <= 10) scores[k].push(val);
        else scores[k].push(5);
      });
      if (ev.violations && Array.isArray(ev.violations) && ev.violations.length > 0) {
        ev.violations.forEach(v => allViolations.push('T'+(i+1)+': '+v));
      }
    } catch(parseErr) {
